...
final class ObjectTests: XCTestCase {
  ...
  func testAmplifierParse() async {
    ...
    await Model.shared.process(.amplifier, status)
    
    if await id != Model.shared.amplifiers[id: id]!.id { XCTFail() }
    if await "ANT1" != Model.shared.amplifiers[id: id]!.ant { XCTFail() }
    if await "10.0.1.106" != Model.shared.amplifiers[id: id]!.ip { XCTFail() }
    if await "PGXL" != Model.shared.amplifiers[id: id]!.model { XCTFail() }
    if await 4123 != Model.shared.amplifiers[id: id]!.port { XCTFail() }
    if await "1234-5678-9012" != Model.shared.amplifiers[id: id]!.serialNumber { XCTFail() }
    if await "STANDBY" != Model.shared.amplifiers[id: id]!.state { XCTFail() }
    
    await Model.shared.amplifiers[id: id]!.update([(.ant, "ANT2"),(.ip,"11.1.217.1"),(.model,"QIYM"),(.port,"3214"), (.serialNumber,"2109-8765-4321"), (.state, "STANDBY")])
    if await "ANT2" != Model.shared.amplifiers[id: id]?.ant { XCTFail() }
    if await "11.1.217.1" != Model.shared.amplifiers[id: id]!.ip { XCTFail() }
    if await "QIYM" != Model.shared.amplifiers[id: id]!.model { XCTFail() }
    if await 3214 != Model.shared.amplifiers[id: id]!.port { XCTFail() }
    if await "2109-8765-4321" != Model.shared.amplifiers[id: id]!.serialNumber { XCTFail() }
    if await "STANDBY" != Model.shared.amplifiers[id: id]!.state { XCTFail() }
    
    var values = [Any]()
    await values = Model.shared.amplifiers[id: id]!.get([.ant, .ip, .model, .port, .serialNumber, .state])
    
    await Model.shared.removeAll(.amplifier)
    
    if await Model.shared.amplifiers[id: id] != nil {
