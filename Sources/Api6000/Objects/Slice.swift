...
extension Slice {
  public func setProperty(radio: Radio, property: Slice.Property, value: Any) {
    ...
    case .filterHigh:               Slice.filterCommand(radio, id, low: Model.shared.slices[id: id]!.filterLow, high: value)
    case .filterLow:                Slice.filterCommand(radio, id, low: value, high: Model.shared.slices[id: id]!.filterHigh)
    ...
    case .frequency:                Slice.tuneCommand(radio, id, value, autoPan: Model.shared.slices[id: id]!.autoPan)
    ...
    Task {
      ...
      case .nbEnabled, .nrEnabled, .anfEnabled, .wnbEnabled, .audioMute, .ritEnabled, .xitEnabled:
        await Model.shared.slices[id: id]?.parse( [(key: property.rawValue, value: "\((value as! Bool).as1or0)")] )
        
      default:
        await Model.shared.slices[id: id]?.parse( [(key: property.rawValue, value: "\(value)")] )
      }
    }
  }
}
