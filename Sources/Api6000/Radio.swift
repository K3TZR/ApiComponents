...
//    objects.equalizers[id: Equalizer.EqType.txsc.rawValue] = Equalizer(Equalizer.EqType.txsc.rawValue)
  ...
  ///         The Client persists the ClientId (if desired))
  ///
  ///     5. The Client connects as a non-Gui, binding is NOT desired - (don't know why you would do this)
  ///         The Client passes clientId = nil, isGui = false to the Api
  ///
  ...
  public func connect(_ packet: Packet) -> Bool {
    guard radioState == .clientDisconnected else {
      log("Radio: Invalid state on connect, apiState != .clientDisconnected", .warning, #function, #file, #line)
      return false
    }
    // attempt to connect
...
//      objects.daxIqStreams[id: vitaPacket.streamId]?.vitaProcessor(vitaPacket)
    ...
    default:
      // log the error
      log("Radio, unknown Vita class code: \(vitaPacket.classCode.description()) Stream Id = \(vitaPacket.streamId.hex)", .error, #function, #file, #line)
    }
  }
  ...
  func updateState(to newState: RadioState) {
    ...
    case .tcpConnected (let host, let port):
      log("Radio: TCP connected to \(host), port \(port)", .debug, #function, #file, #line)
    ...
    case .wanHandleValidated (let success):
      if success {
        log("Radio: Wan handle validated", .debug, #function, #file, #line)
        if _udp.bind(packet) == false { _tcp.disconnect() }
      } else {
        log("Radio: Wan handle validation FAILED", .debug, #function, #file, #line)
        _tcp.disconnect()
      }
      
    case .udpBound (let receivePort, let sendPort):
      log("Radio: UDP bound, receive port = \(receivePort), send port = \(sendPort)", .debug, #function, #file, #line)
    ...
    case .clientConnected:
      log("Radio: client connected (LOCAL)", .debug, #function, #file, #line)
    ...
    case .tcpDisconnected (let reason):
      log("Radio: Tcp Disconnected, \(reason ?? "User initiated")", .debug, #function, #file, #line)
    ...
    case .udpUnbound (let reason):
      log("Radio: UDP unbound, \(reason ?? "User initiated")", .debug, #function, #file, #line)
      updateState(to: .clientDisconnected)
      
    case .clientDisconnected:
      log("Radio: Client disconnected", .debug, #function, #file, #line)
      
      // Not Implemented ------------------------------------------------------------------------
    case .update:
      log("Radio: Firmware Update not implemented", .warning, #function, #file, #line)
      break
    }
  }
  ...
  private func clientIpReplyHandler(_ command: String, seqNum: UInt, responseValue: String, reply: String) {
    log("Radio: clientIpReplyHandler for c\(seqNum)|\(command), reply = \(reply)", .debug, #function, #file, #line)
    connectionCompletion()
  }
  ...
  func connectionCompletion() {
    ...
    if _disconnectHandle == nil {
      ...
      if pingerEnabled {
        ...
        log("Radio: pinger started, Radio = \(self)", .info, #function, #file, #line)
      }
  ...
  func sendCommands() {
    if _connectionType == .gui { send("client gui \(_clientId ?? "")")  }
