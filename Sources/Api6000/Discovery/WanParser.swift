...
extension WanListener {
  ...
  func parseVitaPayload(_ text: String) {
    ...
    guard let token = MessageTokens(rawValue: properties[0].key)  else {
      // log it
      log("Wan Parser: \(msg)", .warning, #function, #file, #line)
      return
    }
  ...
  private func parseApplication(_ properties: KeyValuesArray) {
    ...
    guard let token = ApplicationTokens(rawValue: properties[0].key)  else {
      ...
      log("Wan Parser: unknown application token:, \(properties[1].key)", .warning, #function, #file, #line)
      return
    }
    switch token {
  ...
  private func parseRadio(_ properties: KeyValuesArray, msg: String) {
    ...
    guard let token = RadioTokens(rawValue: properties[0].key)  else {
      // log it and ignore the message
      log("Wan Parser: unknown radio token: \(properties[1].key)", .warning, #function, #file,#line)
      return
    }
  ...
  private func parseRegistrationInvalid(_ properties: KeyValuesArray) {
        log("Wan Parser: invalid registration: \(properties.count == 3 ? properties[2].key : "")", .warning, #function, #file, #line)
  }
  ...
  private func parseUserSettings(_ properties: KeyValuesArray) {
    ...
    log("Wan Parser: UserSettings received", .debug, #function, #file, #line)
    ...
    for property in properties {
      ...
      guard let token = UserSettingsTokens(rawValue: property.key)  else {
        // log it and ignore the Key
        log("Wan Parser: unknown settings token: \(property.key)", .warning, #function, #file, #line)
        continue
      }
      // Known tokens, in alphabetical order
      switch token {
  ...
  private func parseRadioConnectReady(_ properties: KeyValuesArray) {
    ...
    log("Wan Parser: ConnectReady received", .debug, #function, #file, #line)
    ...
    for property in properties {
      ...
      guard let token = ConnectReadyTokens(rawValue: property.key)  else {
        // log it and ignore the Key
        log("Wan Parser: unknown connect token: \(property.key)", .warning, #function, #file, #line)
        continue
      }
      // Known tokens, in alphabetical order
      switch token {
  ...
  private func parseTestConnectionResults(_ properties: KeyValuesArray) {
    ...
    for property in properties {
      ...
      guard let token = TestConnectionTokens(rawValue: property.key)  else {
        // log it and ignore the Key
        log("Wan Parser: unknown testConnection token: \(property.key)", .warning, #function, #file, #line)
        continue
      }
    ...
    log("Wan Parser: smartlink test result received, \(result.success ? "SUCCESS" : "FAILURE")", result.success ? .debug : .warning, #function, #file, #line)
    // publish the result
//    Discovered.shared.testPublisher.send(result)
    NotificationCenter.default.post(name: testNotification, object: result as Any?)
  }
}
