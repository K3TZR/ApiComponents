...
final public class Udp: NSObject {
  ...
  public init(receivePort: UInt16 = 4991) {
    ...
    log("UdpStream: socket initialized", .debug, #function, #file, #line)
  }
  ...
  public func bind(_ packet: Packet) -> Bool {
    ...
    for _ in 0..<tries {
      do {
        ...
        log("UdpStream: bound to port, \(portToUse)", .debug, #function, #file, #line)
        success = true
        
      } catch {
        // We didn't get the port we wanted
        log("UdpStream: FAILED to bind to port, \(portToUse)", .debug, #function, #file, #line)
  ...
  public func register(clientHandle: Handle?) {
    guard clientHandle != nil else {
      // should not happen
      log("UdpStream: No client handle in register UDP", .error, #function, #file, #line)
      return
    }
    ...
    _registerQ.async { [unowned self] in
      while self._socket != nil && !self._isRegistered && self._isBound {
        ...
        log("UdpStream: register wan, handle = " + clientHandle!.hex, .debug, #function, #file, #line)
